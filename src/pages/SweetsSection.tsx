...
import { sweetRecipes } from '../data/sweetRecipes';

const SweetsSection = () => {
  const [currentSlide, setCurrentSlide] = useState(0);
  const [searchQuery, setSearchQuery] = useState('');
  const [filteredRecipes, setFilteredRecipes] = useState(Object.entries(sweetRecipes));
  const [searchParams] = useSearchParams();

  const recipesArray = Object.entries(sweetRecipes);
        ...
        <div className="max-w-6xl mx-auto px-5 lg:px-7">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-6">
              ...
              <Link to="/nonveg" className="text-white/80 hover:text-brand-primary text-sm font-medium transition-colors">Non-Veg</Link>
      ...
      <div className="pt-24 pb-16 px-5 lg:px-7">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <p className="text-lg md:text-xl text-gray-300 max-w-3xl mx-auto leading-relaxed animate-fade-in delay-300 mb-6">
            "Indulge in heavenly sweetness – where every bite is a moment of pure bliss and joy!"
          </p>
          ...
          <div className="max-w-sm mx-auto mb-6">
            <div className="relative">
              <Input
                type="text"
                placeholder="Search sweet recipes..."
                value={searchQuery}
                onChange={(e) => {
          ...
          <div className="mt-12 relative">
            <div className="relative w-64 h-64 mx-auto rounded-3xl overflow-hidden shadow-2xl">
              <img
                src="index-S-1c.jpg"
                alt="Sweet delicacies"
                className="w-full h-full object-cover"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent"></div>
              <div className="absolute bottom-5 left-5 right-5 text-center">
                <h3 className="text-2xl font-bold text-white font-playfair">sweets</h3>
              </div>
            </div>
          </div>
        </div>
