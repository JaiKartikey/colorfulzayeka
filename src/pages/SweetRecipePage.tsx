...
const SweetRecipePage = () => {
  const { slug } = useParams();
  const [activeTab, setActiveTab] = useState('ingredients');
  const [servings, setServings] = useState(4);
  const [isFavorited, setIsFavorited] = useState(false);

  const recipe = sweetRecipes[slug as string];
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/sweets" className="text-brand-primary hover:text-brand-accent text-sm">
            Back to Sweet Section
          </Link>
        </div>
      </div>
    );
  ...
  const otherRecipes = Object.entries(sweetRecipes)
    .filter(([key]) => key !== slug)
    .slice(0, 3);
...
export default SweetRecipePage;
